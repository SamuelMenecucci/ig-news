...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === "POST") {
    ...
    const stripeCheckoutSession = await stripe.checkout.sessions.create({
      ...
      success_url: "localhost:3000/posts", //url that the user will be redirected if the payment has been made
      cancel_url: "localhost:3000/", //url that the user will be redirected if the payment has be canceled
    });
